...
const DataconnectorDetail = ({ history, match }) => {
  ...
  const startProject = async (id, type) => {
    ...
    await api
      ...
      .then((res) => {
        dispatch(
          openSuccessSnackbarRequestAction(t("A new project has been created."))
        );
            ...
            <span style={{ marginLeft: 8 }}>
              ...
              {t(
                connectorInfo.dataconnectortype?.dataconnectortypeName === "CSV"
                  ? "This is a sample of your data (up to 120 rows)."
                  : "It is shown as sample data (within 20 sheets)."
              )}{" "}
              ]
            </span>
          )}
