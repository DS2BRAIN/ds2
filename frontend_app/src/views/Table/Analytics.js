          ...
          <>
            {prescriptionAnalyticsInfo &&
              Object.keys(prescriptionAnalyticsInfo).length > 0 && (
                ...
                >
                  [{projects?.project?.valueForPredict.split("__")[0]}]{" "}
                  {t("Goal Value")} :
                  <Select defaultValue={0} onChange={valueInfoKeyChange}>
                    {Object.keys(prescriptionAnalyticsInfo).map(
                      (infoKey, index) => {
                        return <MenuItem value={index}>{infoKey}</MenuItem>;
                      }
                    )}
                  </Select>
                  <GridContainer>
                    ...
                    >
                      <div
                        dangerouslySetInnerHTML={{
                          __html:
                            i18n.language === "en"
                              ? prescriptionAnalyticsInfo[selectedPAname][
                                  "sentence_en"
                                ]
                              : prescriptionAnalyticsInfo[selectedPAname][
                                  "sentence_ko"
