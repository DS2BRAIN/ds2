...
const SummaryTable = React.memo(
  ({
    category,
    csv,
    getTimeSeriesCheckedValue,
    getCheckedValue,
    getProcessingInfo,
    getProcessingInfoValue,
    trainingColumnInfo,
    timeSeriesColumnInfo,
    preprocessingInfoParent,
    preprocessingInfoValueParent,
    handleIsTimeSeries,
  }) => {
    ...
    const [
      tempPreprocessingInfoValue,
      setTempPreprocessingInfoValue,
    ] = useState({});
    const [isPreprocessingModalOpen, setIsPreprocessingModalOpen] = useState(
      false
    );
    const [
      selectedRowForPreprocessing,
      setSelectedRowForPreprocessing,
    ] = useState([]);
    ...
    const makeCheckValue = () => {
      ...
      if (
        preprocessingInfoParent &&
        Object.keys(preprocessingInfoParent).length === 0
      ) {
        for (let i = 0; i < csv.length; i++) {
          const value = csv[i].id;
    ...
    const onSetSummaryCheckedValue = async (value) => {
      ...
      if (category === "sample") {
        dispatch(
          openErrorSnackbarRequestAction(
            t(
              "You can’t make changes to 'Training data usage' when using sample data."
            )
          )
        );
        return;
      ...
      if (projects.project.status === 100) {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can’t make changes to a completed project")
          )
        );
        return;
      }
      if (projects.project.status !== 0) {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can’t make changes to an ongoing project")
          )
        );
        return;
      }
    ...
    const onSetPreprocessingCheckAll = async (value) => {
      ...
      if (category === "sample") {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can’t make changes to sample projects")
          )
        );
        return;
      }
      if (projects.project.status === 100) {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can’t make changes to completed projects")
          )
        );
        return;
      }
      if (projects.project.status !== 0) {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can’t make changes to ongoing projects")
          )
        );
        return;
      }
      ...
      rows.map((row, idx) => {
        preprocessingCheckedValueAll[
          row[row.length - 1]
        ] = !isAllpreprocessingChecked;
      });
    ...
    const onSetTimeSeriesCheckedValue = async (value, dataconnectorId) => {
      ...
      if (timeSeriesCheckedDict[dataconnectorId] === value) {
        dispatch(
          openErrorSnackbarRequestAction(
            t("The time series criterion requires one column per data.")
          )
        );
        return;
      } else {
    ...
    const setTimeSeriesCheckValueFunc = (value) => {
      ...
      if (projects.project.status === 100) {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can't change time series criteria for completed projects")
          )
        );
        return;
      }
      if (projects.project.status !== 0) {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can't change time series criteria for ongoing projects")
          )
        );
        return;
      }
      if (projects.project.statusText === "중단") {
        dispatch(
          openErrorSnackbarRequestAction(
            t("You can't change time series criteria for suspended projects")
          )
        );
        return;
      }
      if (!timeSeriesCheckedValue.hasOwnProperty(value)) {
        dispatch(
          openErrorSnackbarRequestAction(
            t(
              "This column cannot be used as a reference for time series forecasting."
            )
          )
        );
        return;
    ...
    const onSetPreprocessingValue = async (value) => {
      setSelectedRowForPreprocessing(
        Object.keys(preprocessingCheckedValue).filter(
          (infoValue) => preprocessingCheckedValue[infoValue] === true
        )
      );
    ...
    const onClickpreprocessingInfoValueValue = async (
      rowId,
      proprocessingOptionId
    ) => {
      var preprocessingCheckedValueTrue = Object.keys(
        preprocessingCheckedValue
      ).filter((infoValue) => preprocessingCheckedValue[infoValue] === true);
        ...
        if (preprocessingCheckedValueTrue.length > 1) {
          ...
          await setPreprocessingInfo((prevState) => {
            return {
              ...
              [+targetId]: {
                ...prevState[+targetId],
                [proprocessingOptionId]: !preprocessingInfo[targetId][
                  proprocessingOptionId
                ],
              },
            };
    ...
    const onClickpreprocessingInfoValueDict = async (e) => {
      ...
      if (type.indexOf("cleaningClassification") !== -1) {
        if (num < 1) {
          dispatch(
            openErrorSnackbarRequestAction(
              t("Data cleansing has a minimum of 1.")
            )
          );
          return;
        }
      ...
      // }
        ...
        if (num < 0) {
          dispatch(
            openErrorSnackbarRequestAction(
              t("Data cleansing has a minimum of zero.")
            )
          );
          return;
        }
      ...
      <GridItem xs={12} style={{ marginTop: "20px" }}>
            ...
            {projects.project.status === 0 &&
              ...
              !(
                ...
                <div style={{ display: "flex" }}>
                  <Button
                    id="preCleansingAllBtn"
                    shape="greenOutlined"
                    disabled={
                      Object.values(preprocessingCheckedValue).filter(
                        (infoValue) => infoValue === true
                      ).length === 0
                    }
                    onClick={onSetPreprocessingValue}
                  >
                    {t("Preprocessing")}
                  </Button>
                  <Tooltip
                    title={
                      <span style={{ fontSize: "12px" }}>
                        {t(
                          "There are three kinds of preprocessing : data cleansing, normalization and replacement. You can choose as needed"
                        )}
                      </span>
                    }
                    placement="right"
        ...
        <div className={classes.tableWrapper}>
          ...
          >
                    ...
                    idx === 0 ? (
                      <React.Fragment key={`column_${idx}`}>
                                  ...
                                  <Tooltip
                                    title={
                                      <span style={{ fontSize: "11px" }}>
                                        {t(
                                          "Select the final value you want to predict."
                                        )}
                                      </span>
                                    }
                                    placement="top"
                                ...
                                >
                                  {projects.project.status < 1 &&
                                    projects.project.statusText !== "중단" && (
                                      <Checkbox
                                        name="cleansingAll"
                                        id="cleansingAllCheckbox"
                                        checked={isAllpreprocessingChecked}
                                        disabled={
                                          projects.project.status !== 0 ||
                                          projects.project.statusText === "중단"
                                        }
                                        onClick={onSetPreprocessingCheckAll}
                              ...
                              >
                                ...
                                <Tooltip
                                  title={
                                    <span style={{ fontSize: "11px" }}>
                                      {t(
                                        "Time series prediction is an AI model that predicts what value the data will have in the future by training given data that changes over time."
                                      )}
                                    </span>
                                  }
                                  placement="top"
                        ...
                        <TableCell
                          ...
                          key={column.label + column.id + idx}
                          align="center"
                          className={classes.tableHead}
                        >
                          <div
                            style={{ color: currentThemeColor.textMediumGrey }}
                          >
                            {t(column.label)}
                          </div>
                        </TableCell>
            ...
            <TableBody>
              {rows.map((row, idx) => {
                var isChecked = summaryCheckedValue[row[row.length - 1]]
                  ? true
                  : false;
                var isPreprocessingChecked = preprocessingCheckedValue[
                  row[row.length - 1]
                ]
                  ? true
                  : false;
                var isTimeSeriesChecked = timeSeriesCheckedValue[
                  row[row.length - 1]
                ]
                  ? true
                  : false;
                var isPreprocessingDone =
                  projects.project.status < 1 &&
                  projects.project.statusText !== "중단"
                    ? preprocessingDoneValue[row[row.length - 1]]
                    : isPreprocessingChecked;
                return (
                  <TableRow
                      ...
                      !(
                        ...
                        <>
                          ...
                          >
                            <Switch
                              value={
                                typeof row[2] === "number" ? row[2] - 1 : 0
                              }
                              checked={isChecked}
                              color="primary"
                              inputProps={{ "aria-label": "primary checkbox" }}
                              onClick={() =>
                                onSetSummaryCheckedValue(row[row.length - 1])
                              }
                            />
                          </TableCell>
                          <TableCell
                          ...
                          >
                            <div className={classes.defaultContainer}>
                              {projects.project.status < 1 &&
                                projects.project.statusText !== "중단" && (
                                  <Checkbox
                                    ...
                                    onClick={() =>
                                      onSetPreprocessingCheckedValue(
                                        row[row.length - 1]
                                      )
                                    }
                                  />
                              ...
                              {isPreprocessingDone ? (
                                <div className={classes.highlightSpan}>
                                  {t("Completed")}
                                </div>
                              ) : projects.project.status < 1 ? null : (
                                <div>-</div>
                              )}
                            </div>
                          </TableCell>
                    ...
                    {projects.project.trainingMethod &&
                      projects.project.trainingMethod.indexOf("time_series") >
                        ...
                        >
                          <Checkbox
                            value={row[2] - 1}
                            checked={isTimeSeriesChecked}
                            onClick={() =>
                              onSetTimeSeriesCheckedValue(
                                row[row.length - 1],
                                row[3]
                              )
                            }
        ...
        <Modal
          ...
          open={isPreprocessingModalOpen}
          onClose={PreprocessingModalActionClose}
          ...
          <div className={classes.preprocessingmodalContent} id="projectModal">
            <div className={classes.gridRoot}>
              <GridContainer style={{ width: "100%" }}>
                <GridItem
                  xs={12}
                  style={{ margin: "0px !important", marginBottom: "30px" }}
                >
              ...
              <GridContainer style={{ width: "100%", paddingBottom: "50px" }}>
                {selectedRowForPreprocessing[6] !== "number" && (
                  ...
                  >
                    <FormControlLabel
                      control={
                        <Checkbox
                          ...
                          checked={
                            preprocessingInfo[
                              selectedRowForPreprocessing[
                                selectedRowForPreprocessing.length - 1
                              ]
                            ] &&
                            preprocessingInfo[
                              selectedRowForPreprocessing[
                            ...
                            ]["cleaningClassification"]
                          }
                          onClick={() =>
                            onClickpreprocessingInfoValueValue(
                              selectedRowForPreprocessing[
                                selectedRowForPreprocessing.length - 1
                              ],
                              "cleaningClassification"
                      ...
                      label={
                        ...
                        preprocessingInfo[
                          ...
                          ? t("Data cleansing")
                          : `${t("Data cleansing")} : ${t(
                              "데이터 개수 n개 미만인 유니크 값을 가지고 있는 행들을 삭제합니다."
                            )}`
                      }
                    />
                    {(preprocessingInfo[
                    ...
                    Object.keys(preprocessingInfo).length === 0 ? (
                      <></>
                    ) : (
                      <b>
                        <TextField
                        ...
                        {t(
                          "Rows with less than () unique values will be deleted"
                        )}
                      </b>
                    )}
                  </GridItem>
                  ...
                  <>
                      ...
                      <FormControlLabel
                        ...
                        label={
                          ...
                          preprocessingInfo[
                            ...
                            ? t("Data cleansing")
                            : t(
                                "Data Cleansing : Delete rows that exceed n% of the total standard distribution."
                              )
                        }
                      />
                      {(preprocessingInfo[
                    ...
                    >
                      <FormControlLabel
                        ...
                        label={`${t("Normalization")} : ${t(
                          "Normalizes the entire value."
                        )}`}
                      />
                    </GridItem>
                  </>
                  ...
                  <FormControlLabel
                    ...
                    label={
                      ...
                      preprocessingInfo[
                        ...
                        : `${t("Remove outliers")} : ${t(
                            "Delete columns in which unique values account for n% or more of the total number of data."
                          )}`
                    }
                  />
                  {(preprocessingInfo[
                  ...
                  <FormControlLabel
                    ...
                    label={
                      ...
                      preprocessingInfo[
                        ...
                        ? t("Data replacement")
                        : `${t("Data replacement")} : ${t(
                            "Fill empty values with n."
                          )}`
                    }
                  />
                  {(preprocessingInfo[
                      ...
                      >
                        ...
                        >
                          ...
                          <MenuItem
                            ...
                            value={t("Median value")}
                          >
                            {t("Median value")}
                          </MenuItem>
                        </Select>
                      </FormControl>{" "}
                      : {t("Fill empty values with")}{" "}
                      <u>
                        {" "}
                        {preprocessingInfoValue &&
                          preprocessingInfoValue[
                      ...
                      projects.project.trainingMethod.indexOf("time_series") >
                        ...
                        >
                          <FormControlLabel
                            ...
                            label={`${t("Time series average")} : ${t(
                              "When converting, time series statistics are calculated from the average value, not the sum value."
                            )}`}
                          />
                        </GridItem>
                      )}
              ...
              <GridContainer style={{ width: "100%" }}>
                ...
                <GridItem xs={4}>
                  <Button
                    ...
                    style={{ width: "100%" }}
                    onClick={PreprocessingSaveClose}
                  >
                    {t("Confirm")}
                  </Button>
                </GridItem>
