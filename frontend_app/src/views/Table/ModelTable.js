...
import { postFavoriteModelRequestAction, putProjectWebhooksRequest, postPurchaseModelRequestAction } from "redux/reducers/projects.js";
...
import { Divider, FormControl, MenuItem, Modal, Select, Table, TableBody, TableCell, TableHead, TablePagination, TableRow, TextField } from "@material-ui/core";
import { withStyles } from "@material-ui/core/styles";
import { Checkbox, CircularProgress, FormControlLabel, Grid, LinearProgress, Popover, Tooltip } from "@mui/material";
...
const ModelTable = React.memo(({ category, csv, trainingColumnInfo, history, projectStatus, price, isAnyModelFinished, isVerify, selectedPage }) => {
  ...
  const modelProcessing = fileurl + "asset/front/img/modelIcon/modelProcessing.png";

  useEffect(() => {
    if (!isSseInitiated && projects.project?.id && (projects.project?.status > 0 || isAnyModelFinished) && projects.project.models.length) {
      setIsLoading(true);
  ...
  }, [projects.project?.id, projects.project?.models, projects.project?.status]);
  ...
  useEffect(() => {
    if (projects.project.webhookMethod) setWebhooksMethod(projects.project.webhookMethod);
  }, [projects.project && projects.project.webhookMethod]);

  useEffect(() => {
    if (projects.project.webhookURL) setWebhooksUrl(projects.project.webhookURL);
  ...
  const initModelData = async (modelsArr) => {
    ...
    for (let idx = 0; idx < models.length; idx++) {
      ...
      if (model.status === 0) modelStatus["waiting"] = modelStatus["waiting"] + 1;
      if (model.status === 1) modelStatus["processing"] = modelStatus["processing"] + 1;
      if (model.status === 9) modelStatus["pausing"] = modelStatus["pausing"] + 1;
      if (model.status === 99) modelStatus["error"] = modelStatus["error"] + 1;
      if (model.status === 100) {
    ...
    if (sortObj[sortValueRef.current] === "up") {
      if (sortValueRef.current === "name") {
        tempModels.sort((prev, next) => {
          return next[sortValueRef.current].localeCompare(prev[sortValueRef.current], undefined, {
            numeric: true,
            sensitivity: "base",
          });
        });
      ...
      if (sortValueRef.current === "name") {
        tempModels.sort((prev, next) => {
          return prev[sortValueRef.current].localeCompare(next[sortValueRef.current], undefined, {
            numeric: true,
            sensitivity: "base",
          });
        });
      ...
      const headCellsBase = () => {
        return (
          <>
            ...
            <TableCell className={classes.tableHead} align="center" style={{ width: "30%", cursor: "pointer" }} onClick={() => onSetSortValue("name")}>
      ...
      const headCellsDataObj = (data) => {
        const renderDataHead = (defaultKey, apKey) => {
          ...
          const defKeyInfo = {
            rmse: {
              ...
              tooltip: "평균 제곱근 오차로써 연속값을 예측할때 사용되는 지표입니다. RMSE 값이 낮을수록 근접하게 예측합니다.",
            },
            totalLoss: {
              name: "Total_loss",
            ...
            accuracy: {
              ...
              tooltip: "모델의 정확도를 나타냅니다. ACCURACY 값이 높을수록 정확하게 예측합니다.",
            },
            errorRate: {
              name: "Error Rate",
              anchor: anchorErrorRate,
              setAnchor: setAnchorErrorRate,
              tooltip: "샘플링을 할 때 생긴 오류의 비율을 나타냅니다. 값이 낮을수록 예측 오류가 나올 확률이 낮아집니다.",
            },
            dice: {
              name: "Dice",
              anchor: anchorDice,
              setAnchor: setAnchorDice,
              tooltip: "실제 값과 예측 값의 유사성을 측정하기 위해 사용되는 샘플 계수입니다. DICE 값이 높을수록 유사성이 높습니다.",
            },
            r2score: {
              name: "R2",
              anchor: anchorR2Score,
              setAnchor: setAnchorR2Score,
              tooltip: "모델이 데이터를 얼마나 잘 설명하는지 나타내는 지표입니다. 0~1 값을 가질 수 있으며, 1에 가까울수록 모델이 데이터와 연관성이 높다고 할 수 있습니다.",
            },
            mase: {
              name: "MAE",
              anchor: anchorMase,
              setAnchor: setAnchorMase,
              tooltip: "예측값과 실제값의 차이의 절대 값을 취하여 해당 평가 예측에 대한 오차를 측정합니다.",
            },
          };
          ...
          let resKey = substituteHead[defaultKey] ? substituteHead[defaultKey] : defaultKey;
            ...
            return (
              <TableCell className={classes.tableHead} align="center" style={{ width: "10%" }}>
                <Grid className={classes.tableHeader}>
                  ...
                  <b style={{ cursor: "pointer" }} onClick={(e) => defKeyInfo[defaultKey]?.setAnchor(e.currentTarget)}>
                    {defKeyInfo[resKey]?.name ? t(defKeyInfo[resKey].name) : resKey}
                  </b>
                  {defKeyInfo[resKey]?.tooltip && (
                    <Tooltip title={<span style={{ fontSize: "11px" }}>{t(defKeyInfo[resKey].tooltip)}</span>} placement="top-start">
                      <HelpOutlineIcon
                        id="helpIcon"
                        style={{
                          marginLeft: "5px",
        ...
        return (
          <>
            ...
            {data.accuracy && projectTrainMethod !== "object_detection" && renderDataHead("accuracy")}
            {data.errorRate && projectTrainMethod !== "object_detection" && projectTrainMethod !== "normal_regression" && renderDataHead("errorRate")}
            {data.dice && renderDataHead("dice")}
            {data.r2score && (projectTrainMethod === "normal_regression" || projectTrainMethod === "time_series_regression") && renderDataHead("r2score")}
          ...
          {models.slice(page * rows, page * rows + rows).map((model, idx) => {
            ...
            const bodyCellsBase = (model, modelDict, rowNum) => (
              <>
                ...
                <TableCell className="tableRowCell" id="modelTable" align="center">
                  ...
                  {model.status === 1 && <LinearProgress className={classes.linearProgressLightBackground} variant="determinate" value={modelDict[model.id]?.progress ? modelDict[model.id].progress : model.progress} />}
                </TableCell>
              </>
            );
            ...
            const bodyCellsDataObj = (model, data) => {
              const renderDataBody = (defaultKey, defaultValue, model) => {
                let resValue = substituteHead[defaultKey] ? model[substituteHead[defaultKey]] : defaultValue;
                return (
              ...
              let rmse = model.rmse ? model.rmse.toFixed(4) : model.rmse === 0 ? 0 : "-";
              let totalLoss = model.totalLoss || model.totalLoss === 0 ? model.totalLoss : "-";
              let accuracy = model.accuracy ? `${(model.accuracy * 100).toFixed(4)}%` : model.accuracy === 0 ? 0 : "-";
              let errorRate = model.errorRate ? model.errorRate : model.errorRate === 0 ? 0 : "-";
              let dice = model.dice ? model.dice : model.dice === 0 ? 0 : "-";
              let r2score = model.r2score ? model.r2score : model.r2score === 0 ? 0 : "-";
              let mase = model.mase ? model.mase : model.mase === 0 ? 0 : "-";

              return (
                <>
                  {data.rmse && renderDataBody("rmse", rmse, model)}
                  {data.totalLoss && renderDataBody("totalLoss", totalLoss, model)}
                  {data.accuracy && projectTrainMethod !== "object_detection" && renderDataBody("accuracy", accuracy, model)}
                  {data.errorRate && projectTrainMethod !== "object_detection" && projectTrainMethod !== "normal_regression" && renderDataBody("errorRate", errorRate, model)}
                  {data.dice && renderDataBody("dice", dice, model)}
                  {data.r2score && (projectTrainMethod === "normal_regression" || projectTrainMethod === "time_series_regression") && renderDataBody("r2score", r2score, model)}
            ...
            const bodyCellStatus100 = (project) => {
              const openDetailPage = (page, modelId, projectId) => {
                if (!Cookies.getCookie("user")) {
                  dispatch(openErrorSnackbarRequestAction(t("User information not found.")));
                  return;
                }
                if (category === "process") {
              ...
              const secPredictBtns = (method, id) => {
                let isImage = method === "object_detection" || method === "cycle_gan" || method === "image";
                let isVideo = method === "object_detection" || method === "cycle_gan";
                ...
                return (
                  <>
                    {isImage ? (
                      <div onClick={() => openModal(id, "apiImage")} className={`${classes.modelTab} imageBtn ${classes.modelTabButton}`}>
                        {t("Image prediction")}
                      </div>
                    ) : (
                      <>
                        <div onClick={() => openModal(id, project.option?.indexOf("load") === -1 ? "api" : "apiLoaded")} className={`${classes.modelTab} apiBtn ${classes.modelTabButton}`}>
                          {t("Single prediction")}
                        </div>
                        {!isRecommender && (
                          <div onClick={() => openModal(id, "predict")} className={`${classes.modelTab} predictBtn ${classes.modelTabButton}`}>
                    ...
                    {isVideo && (
                      <div onClick={() => openModal(id, "apiVideo")} className={`${classes.modelTab} videoBtn ${classes.modelTabButton}`}>
                        {t("Video prediction")}
                      </div>
                    )}
                  </>
              ...
              return (
                ...
                >
                  <div onClick={() => openDetailPage("detail", id, project.id)} className={`${classes.modelTab} detailBtn ${classes.modelTabButton}`}>
                    {t("Details")}
                  </div>
  ...
  const onSendWebhooks = () => {
    ...
    if (webhooksUrl.match(urlRegExp) === null) {
      dispatch(openErrorSnackbarRequestAction(t("Please enter the correct URL.")));
      return;
    }
    dispatch(
    ...
    <div style={{ width: "100%" }}>
      <div style={{ overflow: "auto" }}>
        {isLoading || (category === "sample" && projectStatus !== 100) ? (
          ...
          <>
            {sortedModels.length > 0 && (projects.project.status !== 99 || (projects.project.status === 99 && isAnyModelFinished)) && (
              <GridContainer
                style={{
                  display: "flex",
                  alignItems: "flex-end",
                  ...
                  {modelStatus && (
                    <>
                      <div className={classes.modelCard}>
                        ...
                        >
                          <BorderLinearProgress variant="determinate" value={modelStatus["total"] === 0 ? 0 : (modelStatus["done"] / modelStatus["total"]) * 100} />
                          <b className={classes.modelProgressbar}>{modelStatus["total"] === 0 ? "0 %" : ((modelStatus["done"] / modelStatus["total"]) * 100).toFixed(1) + " %"}</b>
                        </div>
                        <GridContainer>
                          {[
                            ...
                            <GridItem xs={3} key={stat.id}>
                              <div className={classes.modelIconContainer}>
                                <div style={{ display: "flex" }}>
                                  <img src={stat.img} className={classes.modelIconImg} />
                                  <b>{t(stat.label)}</b>
                                </div>
                                <div style={{ display: "flex" }}>
                                  <span className={classes.progressFont}>{modelStatus[stat.id]}</span>
                                  <span className={classes.progressSamllFont}>/{modelStatus["total"]}</span>
                                </div>
                              </div>
                            </GridItem>
                          ))}
            ...
            <GridContainer>
              {projects.project.status !== 99 || (projects.project.status === 99 && isAnyModelFinished) ? (
                showModelTable()
              ) : (
                <>
                  {(projects.project.status === 99 || projects.project.status === 100) && (
                    <div style={{ marginLeft: "20px", marginTop: "24px" }}>
                      ...
                      <br />
                      <div>
                        <b>1. {t("When training data is inserted in a wrong format, an error may occur")}</b>
                      </div>
                      <div>
                        ⇒{" "}
                        {user.language === "ko" ? (
                          <>
                            <span>학습 데이터를 어떻게 넣어야 하는지에 대해서는 </span>
                            <span>
                              <a title="사용자 가이드" href="https://krdocs.ds2.ai/" target="_blank" style={{ textDecoration: "underline" }}>
                                사용자 가이드
                              </a>
                            </span>
                          ...
                          <>
                            ...
                            <span>
                              <a title="the user guide" href="https://docs.ds2.ai/" target="_blank">
                                the user guide
                              </a>
                            </span>
                            <span> for information on how to insert learning data.</span>
                          </>
                        )}
                      </div>
                      <br />
      ...
      >
        <div className={classes.predictModalContent}>
          ...
          <Grid sx={{ fontSize: "24px", ml: 3, mb: 1 }}>{t("Purchase a model license")}</Grid>
          ...
          >
            <FormControlLabel
              ...
              control={<Checkbox value={isBuyingJetson} checked={isBuyingJetson} onChange={onChangeBuyingJetson} color="primary" style={{ marginRight: "10px" }} />}
              label={`${t("Option")} : ${t("Jetson Nano 2GB Developer Kit 에 함께 구매 및 설치 상담 받기 (별도 금액)")}`}
            />
            <a target={"_blank"} href={"https://developer.nvidia.com/embedded/jetson-nano-2gb-developer-kit"} style={{ fontSize: "16px" }}>
          ...
          >
            <FormControlLabel
              ...
              control={<Checkbox value={isCheckedTerm} checked={isCheckedTerm} onChange={onChangeCheckingTerm} color="primary" style={{ marginRight: "10px" }} />}
              label={`${t("I have confirmed the above and the integrated terms of use.")}`}
            />
            <a target={"_blank"} href={lang === "en" ? "https://ds2.ai/terms_of_services.html" : "https://ko.ds2.ai//terms_of_services.html"} style={{ fontSize: "16px" }}>
          ...
          >
            ...
            <GridItem xs={6}>
              {price > 0 ? (
                <Button id="purchase_model_btn" shape="greenOutlined" style={{ width: "80%" }} disabled={!isCheckedTerm} onClick={() => buyDownloadLicense()}>
                  {t("Buy")}
                </Button>
              ) : (
                !IS_ENTERPRISE && (
                  <Button id="contact_cs_btn" shape="greenContained" style={{ width: "80%" }} onClick={openChat}>
                    {t("Contact us")}
                  </Button>
                )
              )}
      ...
      <Modal aria-labelledby="simple-modal-title" aria-describedby="simple-modal-description" open={openWebhooksModal} onClose={closeWebhooksModal} className={classes.modalContainer}>
        <div className={classes.modalContent}>
          <GridContainer xs={12}>
            ...
            <GridItem xs={1} style={{ padding: "0px" }}>
              <CloseIcon id="closeWebhooksModal" className={classes.modalCloseIcon} onClick={closeWebhooksModal} />
            </GridItem>
          </GridContainer>
          <GridContainer style={{ marginTop: "20px" }}>
            ...
            <GridItem xs={4}>
              <FormControl style={{ width: "100%" }}>
                <Select labelid="demo-simple-select-outlined-label" id="demo-simple-select-outlined" value={webhooksMethod} onChange={onWebhooksMethodChange}>
          ...
          <GridContainer style={{ marginTop: "15px" }}>
            ...
            <GridItem xs={9}>
              <TextField id="url" placeholder={t("Please enter the URL.")} value={webhooksUrl} style={{ width: "100%" }} onChange={onChangeWebhooksUrl} />
            </GridItem>
          </GridContainer>
          <GridContainer style={{ marginTop: "30px" }}>
            <GridItem xs={6}>
              <Button id="close_webhookmodal_btn" shape="whiteOutlined" style={{ width: "100%" }} onClick={closeWebhooksModal}>
                {t("Cancel")}
              </Button>
            </GridItem>
            <GridItem xs={6}>
              <Button id="send_webhook_btn" shape="greenOutlined" style={{ width: "100%" }} onClick={onSendWebhooks}>
                {t("Confirm")}
              </Button>
            </GridItem>
          </GridContainer>
