...
const EngineAILists = ({ history }) => {
  ...
  const pollyPredict = async () => {
    await api
      .postMarketTextApi(inputText, selectedOcrModel.id)
      .then((res) => {
  ...
  const imagePredict = async () => {
    await api
      .postMarketImageApi(uploadFile[0], selectedOcrModel.id)
      .then((res) => {
        setObjectJson(res.data);
        dispatch(updatePredictCountRequestAction());
      })
  ...
  const showOCRTable = () => {
    return (
      <>
        ...
        >
          <GridItem xs={6} lg={8}>
            <b>** {t("External AI consumes 1100 units per prediction.")}</b>
          </GridItem>
...
export default React.memo(EngineAILists);
