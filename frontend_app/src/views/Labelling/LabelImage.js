...
const LabelImage = ({ history }) => {
  ...
  const [workapp, setWorkapp] = useState("image");
  ...
  // fileHistoryBy 쿠키 삭제 로직으로 여러 창 열었을 때 삭제되면 안되므로 임시 보류
  // useEffect(() => {
  //   let tmpFileIdCookies = [];

  //   document.cookie
  //     .split(" ")
  //     .filter((v) => v.includes("fileHistoryBy"))
  //     .map((v) => {
  //       tmpFileIdCookies.push(v.split("=")[0]);
  //     });

  //   tmpFileIdCookies.map((v) => {
  //     if (!v.includes(timeStamp)) Cookies.deleteCookie(v);
  //   });
  // }, []);
  ...
  useEffect(() => {
    if (labelFileDetail.labelData) {
      setSelectedValueName(labelFileDetail.labelData);
    }
  }, [labelFileDetail.labelData]);

  useEffect(() => {
    if (isSavingFail) {
      dispatch(openErrorSnackbarRequestAction(t("Failed to save the file.")));
    }
  }, [isSavingFail]);
  ...
  const onGoToSelectedPage = (id) => {
    if (window.confirm(t("진행중이던 라벨링 정보는 초기화 됩니다. 선택하신 이미지로 이동하시겠습니까?"))) {
      setSelectedValueName("");
  ...
  const saveLabel = async (buttonType) => {
    if (checkSaveNum === 0) {
      ...
      if (selectedValueId !== 0) {
        // 기존에 저장된 라벨이 있을 때
        if (Object.keys(labelFileDetail.labelData).length > 0) {
          if (selectedValueName !== labelFileDetail.labelData) {
...
export default React.memo(LabelImage);
