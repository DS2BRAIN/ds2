...
import { LineChart, Line, CartesianGrid, XAxis, YAxis, Tooltip, Legend, BarChart, Bar, ReferenceLine } from "recharts";
...
const Chart = React.memo(({ chosenChart, modelDetail }) => {
  ...
  const { projects, models } = useSelector((state) => ({ projects: state.projects, models: state.models }), []);
  ...
  useEffect(() => {
    ...
    try {
      ...
      for (let idx = 0; idx < length; idx++) {
        if (typeof imp[idx] === "number") {
          ...
          if (parseInt(percentage) > featureMaxMin.max) featureMaxMin.max = parseInt(percentage);
          if (parseInt(percentage) < featureMaxMin.min) featureMaxMin.min = parseInt(percentage);
        }
      }
    ...
    try {
      const tempArr = [];
      if (modelDetail.accuracy) tempArr.push({ name: "Accuracy", value: modelDetail.accuracy });
      if (modelDetail.errorRate) tempArr.push({ name: "ErrorRate", value: modelDetail.errorRate });
      if (modelDetail.dice) tempArr.push({ name: "Dice", value: modelDetail.dice });
  ...
  useEffect(() => {
    if (projects.project.trainingMethod) {
      if (projects.project.trainingMethod === "time_series_regression") {
        try {
          const tempArr = [];
          if (modelDetail.rmse) tempArr.push({ name: "RMSE", value: modelDetail.rmse });
          if (modelDetail.errorRate) tempArr.push({ name: "Error Rate", value: modelDetail.errorRate });
          if (modelDetail.mase) tempArr.push({ name: "MAE", value: modelDetail.mase });
          if (tempArr.length > 0) setDetailData(tempArr);
        } catch {
          setDetailData(null);
        }
    ...
    if (projects.project.trainingMethod) {
      if (projects.project.trainingMethod === "normal_regression") {
        try {
          const tempArr = [];
          if (modelDetail.rmse) tempArr.push({ name: "RMSE", value: modelDetail.rmse });
          if (modelDetail.r2score !== null) tempArr.push({ name: "R2", value: modelDetail.r2score });
          if (modelDetail.mase) tempArr.push({ name: "MAE", value: modelDetail.mase });
          if (tempArr.length > 0) setDetailData(tempArr);
        } catch {
          setDetailData(null);
  ...
  useEffect(() => {
    if (projects.project.trainingMethod && projects.project.trainingMethod === "cycle_gan") {
      try {
        const tempArr = [];
        if (modelDetail.totalLoss) tempArr.push({ name: "Total_Loss", value: modelDetail.totalLoss });
        if (modelDetail.errorRate) tempArr.push({ name: "ErrorRate", value: modelDetail.errorRate });
        if (modelDetail.dice) tempArr.push({ name: "Dice", value: modelDetail.dice });
        if (tempArr.length > 0) setDetailData(tempArr);
      } catch {
  ...
  useEffect(() => {
    if (projects.project.trainingMethod && projects.project.trainingMethod === "object_detection" && modelDetail.ap_info) {
      try {
        const tempArr = [];
        if (modelDetail.ap_info.AP) tempArr.push({ name: "AP", value: modelDetail.ap_info.AP });
        if (modelDetail.ap_info.AP50) tempArr.push({ name: "AP50", value: modelDetail.ap_info.AP50 });
        if (modelDetail.ap_info.AP75) tempArr.push({ name: "AP75", value: modelDetail.ap_info.AP75 });
        if (modelDetail.ap_info.APl) tempArr.push({ name: "APl", value: modelDetail.ap_info.APl });
        if (modelDetail.ap_info.APm) tempArr.push({ name: "APm", value: modelDetail.ap_info.APm });
        if (modelDetail.ap_info.APs) tempArr.push({ name: "APs", value: modelDetail.ap_info.APs });
        if (tempArr.length > 0) setDetailData(tempArr);
      } catch {
        setDetailData(null);
      }
  ...
  const renderChart = (chosenChart) => {
    const colorpalatte = [currentThemeColor.primary1, currentThemeColor.secondary1, COLOR_ORANGE];
    const CustomTooltip = ({ active, payload, label }) => {
      if (active && payload && payload.length !== 0) {
        return (
          <div className="custom-tooltip" style={{ background: "white", color: COLOR_GRAY }}>
    ...
    const TwoLineTooltip = ({ active, payload, label }) => {
      ...
      if (active && payload && payload.length !== 0) {
        return (
          <div className="custom-tooltip" style={{ background: BACKGROUND_COLOR, color: COLOR_BLUE }}>
            <p className="label" style={{ padding: "10px 10px 0 10px" }}>{`EPOCH : ${payload[0].payload.x}`}</p>
            <p className="label" style={{ padding: "0 10px" }}>{`${y1} : ${payload[0].value}`}</p>
            <p className="label" style={{ padding: "0 10px 10px 10px" }}>{`${y2} : ${payload[1].value}`}</p>
          </div>
        );
      }
      return null;
    ...
    const chartWidth = window.innerWidth < 1100 ? 600 : window.innerWidth < 1500 ? 900 : 1300;
    switch (chosenChart) {
      case "detail":
        return (
          <GridContainer style={{ display: "flex" }}>
            ...
            <GridItem xs={12} style={{ marginTop: "20px" }}>
              {isMinusInDetailData ? (
                <Grid style={{ display: "flex" }}>
                  {detailData.map((detailDatum) => (
                    <BarChart width={1000 / detailData.length} height={400} data={[detailDatum]}>
                      <Tooltip content={<CustomTooltip />} cursor={{ fill: BACKGROUND_COLOR }} />
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis dataKey="name" interval={0} height={100} />
                      {detailDatum.value < 0 ? <YAxis domain={[detailDatum.value.toFixed(0) * 1.5, 0]} /> : <YAxis domain={[0, detailDatum.value.toFixed(0) * 2]} />}
                      <Bar dataKey="value" type="monotone" barSize={50} fill={currentThemeColor.primary2} />
                    </BarChart>
                  ))}
                </Grid>
              ) : (
              ...
              ) : projects.project.trainingMethod === "object_detection" ? null : (
                <div style={{ marginLeft: "40px" }}>
                  <div>
                    Accuracy : {t("Indicates how accurate the model is.")}{" "}
                    <a href={"https://en.wikipedia.org/wiki/Accuracy_and_precision"} target={"_blank"} style={{ fontSize: "1rem" }}>
                      ({t("See details")})
                    </a>
                  </div>
                  <div>
      ...
      case "featureImportance":
        const height = featureImportance.length > 6 ? featureImportance.length * 50 : 400;
        return (
          <div
            style={{
              display: "flex",
          ...
          >
            ...
            >
              ...
              <YAxis dataKey="name" stroke={currentThemeColor.textWhite87} interval={0} width={600} height={100} type="category" />
              <XAxis type="number" stroke={currentThemeColor.textWhite87} domain={[featureMaxMin.min, featureMaxMin.max]} />
              <ReferenceLine x={0} stroke="rgb(24, 160, 251)" label="0" strokeWidth={3} alwaysShow={true} />
              <Bar dataKey="value" type="monotone" barSize={25} fill={COLOR_BLUE} />
            </BarChart>
          </div>
        );
      case "loss":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={trainingValidLoss} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="Training_Loss" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="Valid_Loss" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
      ...
      case "precision-recall":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={precisionRecall} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="Precision" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="Recall" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
                <XAxis dataKey="x" interval={0} height={100} />
                <YAxis domain={[0, 1]} />
              </LineChart>
      ...
      case "kappa-coeff":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={kappaMatthew} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="Kappa_score" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="Matthews_corrcoef" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
                <XAxis dataKey="x" interval={0} height={100} />
                <YAxis domain={[0, 1]} />
              </LineChart>
      ...
      case "auroc-fbeta":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={aurocFBeta} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="Auroc" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="F_beta" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
                <XAxis dataKey="x" interval={0} height={100} />
                <YAxis domain={[0, 1]} />
              </LineChart>
                  ...
                  <TableHead>
                    <TableRow>
                      {recordsHeader.map((r, idx) => {
                        ...
                        return (
                          <TableCell align="center" className={classes.tableHead}>
                            <div className={r.length > 30 ? classes.breakText : classes.textInLine}>
                              <b>{r}</b>
                            </div>
                          </TableCell>
                        );
      ...
      case "dChart":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={lossD} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="LossDA" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="LossDB" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
                <XAxis dataKey="x" interval={0} height={100} />
                {/* <YAxis domain={[0, 1]}/> */}
              </LineChart>
      ...
      case "gChart":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={lossG} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="LossGA" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="LossGB" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
                <XAxis dataKey="x" interval={0} height={100} />
                {/* <YAxis domain={[0, 1]}/> */}
              </LineChart>
      ...
      case "idtChart":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={lossIdt} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="LossIdtA" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="LossIdtB" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
                <XAxis dataKey="x" interval={0} height={100} />
                {/* <YAxis domain={[0, 1]}/> */}
              </LineChart>
      ...
      case "cycleChart":
        return (
          <div style={{ display: "flex", marginTop: "40px" }}>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <LineChart width={chartWidth} height={400} data={lossCycle} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                <Tooltip content={<TwoLineTooltip />} />
                <Legend verticalAlign="top" height={36} />
                <Line name="LossCycleA" type="monotone" dataKey="y1" stroke={COLOR_ORANGE} activeDot={{ r: 8 }} />
                <Line name="LossCycleB" type="monotone" dataKey="y2" stroke={COLOR_BLUE} activeDot={{ r: 8 }} />
                <CartesianGrid stroke={COLOR_GRAY} strokeDasharray="3 3" />
                <XAxis dataKey="x" interval={0} height={100} />
                {/* <YAxis domain={[0, 1]}/> */}
              </LineChart>
  ...
  return <div style={{ width: "100%", margin: "0 auto" }}>{renderChart(chosenChart)}</div>;
});
