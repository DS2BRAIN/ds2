          ...
          .catch((e) => {
            ...
            if (e.response && e.response.status === 429) {
              dispatch(
                openErrorSnackbarRequestAction(
                  t(
                    "API 요청이 초과되어 자동로그아웃 되었으니, 다시 로그인해주세요."
                  )
                )
              );
              setTimeout(() => {
            ...
            if (e.response && e.response.data.message) {
              ...
              dispatch(
                openErrorSnackbarRequestAction(
                  t(
                    "업로드하신 영상을 분석하지 못했습니다. 다른 영상파일을 업로드해주세요."
                  )
                )
              );
            }
            ...
            .catch((e) => {
              ...
              if (e.response && e.response.data.message) {
                ...
                dispatch(
                  openErrorSnackbarRequestAction(
                    t(
                      "업로드하신 이미지를 분석하지 못했습니다. 다른 이미지 파일을 업로드해주세요."
                    )
                  )
                );
              }
          ...
          .catch((e) => {
            ...
            if (e.response && e.response.data.message) {
              ...
              dispatch(
                openErrorSnackbarRequestAction(
                  t(
                    "입력값 오류로 인하여 예측에 실패하였습니다. 데이터를 재입력해주세요."
                  )
                )
              );
            }
        ...
        return (
          <div id={`${type}Predict`}>
            <DropzoneArea
              ...
              dropzoneText={t(
                `드래그 앤 드롭으로 ${typeGuideText} 파일을 업로드해주세요.`
              )}
              filesLimit={1}
              maxWidth={"xs"}
              fullWidth={false}
            ...
            if (val) {
              if (year < 1970) {
                ...
                if (!isValidDate) {
                  dispatch(
                    openErrorSnackbarRequestAction(
                      t("It can be set to a point in time earlier than the present.")
                    )
                  );
                  setLocalDateTime(localISOTime);
            ...
            <Grid container item xs={12} style={{ marginTop: "10px" }}>
              ...
              <Grid style={{ fontSize: "8px" }} item container>
                {projects.project?.service_type.indexOf("_training") !== -1
                  ? "*" +
                    t(
                      "영상 시작점 기준으로 10분 길이에 해당되는 영상만 사용됩니다."
                    )
                  : "*" +
                    t(
                      "영상 시작점 기준으로 30분 길이에 해당되는 영상만 사용됩니다."
          ...
          return (
            <>
              ...
              <p>
                *{" "}
                {t(
                  "빨간색은 부정(negative), 푸른색은 긍정(positive)으로 예측이 적용된 결과입니다."
                )}
              </p>
            </>
          );
