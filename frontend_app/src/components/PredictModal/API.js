...
import { Bar, BarChart, CartesianGrid, Legend, ReferenceLine, Tooltip, XAxis, YAxis, ResponsiveContainer } from "recharts";
import { Checkbox, Grid, LinearProgress, Table, TableBody, TableCell, TableHead, TableRow, TablePagination, TextField } from "@material-ui/core";
...
const API = React.memo(({ isStandard, chosenItem, csv, trainingColumnInfo, modelDetail, history, isMarket, opsId, closeModal, marketProjectId, isStandardMovie }) => {
  ...
  const [isCheckedForSettingCreation, setIsCheckedForSettingCreation] = useState(false);
  ...
  useEffect(() => {
    if (csv !== undefined && trainingColumnInfo !== undefined) {
      try {
        setValidCsvCheck(csv?.filter((c) => trainingColumnInfo[c.id + ""] == true));
      } catch (err) {}
    }
  ...
  const sendAPI = (_, predictingCount = 0) => {
    const caseItemApiVideo = () => {
      if (projects.project?.service_type !== undefined) {
        if (projects?.project?.service_type !== null && projects?.project?.service_type.indexOf("_training") !== -1 && syncValue == "") {
          dispatch(openErrorSnackbarRequestAction(t("Enter the video start point.")));
          return;
        }

        if (isCheckedForSettingCreation && !vidCreatedDateTime) {
          dispatch(openErrorSnackbarRequestAction(t("Please specify the video creation date and time.")));
          return;
        }
      }
      ...
      setCompleted(5);
      api
        .predictVideo(models.chosenModel, files, isMarket, opsId, marketProjectId, isStandardMovie, parseFloat(syncValue == undefined || syncValue == null || syncValue == "" ? "0.000" : syncValue), vidCreatedDateTime)
        .then((res) => {
    ...
    const caseTrainMethodImage = () => {
      if (randomFile) {
        ...
        setCompleted(5);
        api
          .predictImageByURL(models.chosenModel, randomFile, "json", isMarket, opsId)
          .then((res) => {
            const response = res.data === null || res.data === undefined ? "오류" : res.data;
            setResponse(response);
            var responseJson = response;
          ...
          .finally(() => {
            if (predictingCount >= 2 || (isPredictImageInfoDone == true && isPredictImageDone)) {
              setApiLoading("done");
            }
            setCompleted(0);
            setLoadingMessage(t("Predicting"));
        ...
        setCompleted(5);
        api
          .predictImageForTextReturn(models.chosenModel, files, isMarket, opsId)
          .then((res) => {
            const response = res.data === null || res.data === undefined ? "오류" : res.data;
            setResponse(response);
            var responseJson = response;
          ...
          .then(() => {
            api
              .predictImageForTextReturnXai(models.chosenModel, files, isMarket, opsId)
              .then((response) => {
    ...
    const caseTrainMethodObjectCycle = () => {
      if (randomFile) {
        ...
        setCompleted(5);
        api
          .predictImageByURL(models.chosenModel, randomFile, "blob", isMarket, opsId)
          .then((response) => {
            return new Blob([response.data]);
          })
          .then(async (blob) => {
            if (projects.project.trainingMethod === "object_detection") {
              ...
              try {
                await api
                  .predictRandomImageInfo(models.chosenModel, blob, isMarket, opsId)
                  .then((res) => {
        ...
        api
          ...
          .then(() => {
            ...
            if (projects.project.trainingMethod === "object_detection") {
              ...
              try {
                api
                  .predictImageInfo(models.chosenModel, files, isMarket, opsId)
                  .then((res) => {
    ...
    const caseTrainMethodEtc = () => {
      if (projects.project && projects.project.trainingMethod !== "recommender") {
        ...
        try {
          ...
          dispatch(openErrorSnackbarRequestAction(t("Please fill in correct data and proceed.")));
          return;
        }
      }
      ...
      Object.keys(paramsValueForPredict).map((eachValue) => {
        paramsValueForPredictRaw[eachValue] = Number.isNaN(+paramsValueForPredict[eachValue]) || paramsValueForPredict[eachValue].length === 0 ? paramsValueForPredict[eachValue] : +paramsValueForPredict[eachValue];
      });
      ...
      if (modelDetail?.outputData_en?.indexOf("Audio") > -1) {
        ...
        .then((res) => {
          ...
          setResponse(response);
          var responseJson = {};
          if (typeof response === "string") responseJson = JSON.parse(response);
          else if (typeof response === "object") responseJson = response;
          if (projects.project.trainingMethod === "recommender" || projects.project.option.indexOf("load") > -1) {
  ...
  const CustomTooltip = ({ active, payload, label }) => {
    if (active && payload && payload.length !== 0) {
      return (
        ...
        >
          <p className="label" style={{ padding: "10px" }}>{`${label} : ${payload[0].value}`}</p>
        </div>
      );
    }
    return null;
  ...
  const getRandomRecords = () => {
    ...
    if (traininMethodType.indexOf(projects.project.trainingMethod) > -1) {
      if (randomFiles.length > 0) {
        ...
        setRandomFile(IS_ENTERPRISE ? fileurl + "static" + randomFiles[newIndex] : randomFiles[newIndex]);
      }
    } else {
      let records = modelDetail.records;
      if (records && records.length > 0) {
        ...
        for (let value in paramsValue) {
          if (records[randomIdx][value] === undefined) {
            tempParams[value] = "";
          } else if (!records[randomIdx][value] && records[randomIdx][value] !== 0) {
          ...
          api
            ...
            .then((res) => {
              if (res.data) {
                ...
                dispatch(openErrorSnackbarRequestAction(t("No random forecasts exist. Please enter data manually.")));
              }
            })
            .catch((e) => {
              if (!process.env.REACT_APP_DEPLOY) console.log(e);
  ...
  const resetImage = () => {
    ...
    if (trainMethod === "image" || models.model.externalAiType === "image" || models.model.externalAiType === "audio" || trainMethod === "object_detection" || trainMethod === "cycle_gan") {
      ...
      if (inputLoadedModelRef.current !== undefined && inputLoadedModelRef.current.value !== undefined) {
  ...
  const renderContents = () => {
          ...
          <Table className={classes.table} aria-label="simple table">
            ...
            <TableBody>
              {Object.keys(paramsValue).map(
                (param, idx) =>
                  ((projects.project && projects.project.trainingMethod === "recommender" && param.indexOf(projects.project.recommenderUserColumn) > -1) || (projects.project && projects.project.trainingMethod !== "recommender")) && (
                    <TableRow key={param + idx} className={classes.tableRow}>
                      ...
                      <TableCell className={classes.tableRowCell} align="center">
                        <TextField
                          ...
                          // multiline={true}
                          required
                          type={paramsType[param] === "string" ? "text" : paramsType[param]}
                        />
                      </TableCell>
                    </TableRow>
                  )
    ...
    const caseApiImage = (acceptedFiles) => {
      if (files && files.length > 0) {
        return (
          ...
          >
            <img src={files[0].preview} style={{ width: "100%", maxHeight: "24em" }} />
          </div>
        );
      } else if (randomFile) {
        return (
          ...
          >
            <img src={randomFile} style={{ width: "100%", maxHeight: "24em" }} />
          </div>
        );
    ...
    const caseApiVideo = (acceptedFiles) => {
      if (files && files.length > 0 && files[0].name.length >= 50) {
        dispatch(openErrorSnackbarRequestAction(t("Set your video title to 50 characters or less.")));
        resetImage();
      }
      const secSetupVidCreationTime = () => {
        ...
        const onChangeVidCreationTime = (e) => {
          ...
          const isValidDate = new Date(val).valueOf() <= new Date(localDateTime).valueOf();
          ...
          if (val) {
            if (year < 1970) {
              ...
              if (!isValidDate) {
                dispatch(openErrorSnackbarRequestAction(t("It can be set to a point in time earlier than the present.")));
                setLocalDateTime(localISOTime);
          ...
          <>
            ...
            <Grid item xs={12}>
              <Grid container alignItems="center">
                <Checkbox color="default" checked={isCheckedForSettingCreation} onChange={(e) => onChangeTimeCheckBox(e)} style={{ marginRight: "4px" }} />
                <Grid item>
      ...
      const secSetupVidStartPoint = () => {
        const onChangeVidStartPoint = (e) => {
          ...
          if (targetVal >= uploadVideoRef.current?.duration) {
            ...
            dispatch(openErrorSnackbarRequestAction(t("The video start point exceeds the video length.")));
          } else if (targetVal.indexOf(".") !== -1) {
            if (/^$|^[0-9]([.]\d{0,3})?$|^[1-9][0-9]*([.]\d{0,3})?$/.test(targetVal) == false) {
              if (uploadVideoRef !== undefined && uploadVideoRef.current !== undefined) {
              ...
              syncRef.current.value = targetVal.substr(0, targetVal.length - 1);
        ...
        const handleCurrentFrame = () => {
          ...
          if (String(uploadVideoRef.current?.currentTime).split(".")[1] == undefined) {
            syncRef.current.value = String(uploadVideoRef.current?.currentTime);
            setSyncValue(syncRef.current.value);
          } else {
            let l = String(uploadVideoRef.current?.currentTime).split(".")[1].length;
            if (l > 3) {
              syncRef.current.value = String(uploadVideoRef.current?.currentTime).substr(0, String(uploadVideoRef.current?.currentTime).length - (l - 3));
              setSyncValue(syncRef.current.value);
            }
          }
        ...
        return (
          <Grid container item xs={12} style={{ marginTop: "10px" }}>
            <div style={{ paddingTop: "5px", marginRight: "10px" }}>{t("Enter video start point")}</div>
            <TextField
              onChange={(e) => {
            ...
            <Grid item container xs={12} justify="center">
              {isStandard && isStandardMovie == false && (
                <Grid container item xs={6} justify="space-between" alignItems="center" className={classes.defaultOutlineButton} style={{ height: "32px", borderRadius: "4px" }}>
                  <Button
                    onClick={() => {
        ...
        return (
          ...
          >
            <Grid container item xs={12} justify="space-between">
              <Grid container item xs={isStandard && isStandardMovie == false ? 6 : 12} justify="center" direction="column">
                {isStandard == false ? (isStandardMovie == true ? t("Standard Video") : t("Current Video")) : t("Current Video")}
                <video ref={uploadVideoRef} style={{ width: "100%", maxHeight: "24em" }} controls autoPlay currentTime={0} src={files[0].preview} type="video/mp4">
              ...
              {isStandard && isStandardMovie == false && (
                <Grid container item xs={6} justify="center" direction="column">
                  {t("Standard Video")}
                  <video ref={standardVideoRef} style={{ width: "100%", maxHeight: "24em" }} controls autoPlay currentTime={0} src={isStandard} type="video/mp4">
                    {/* <source id="videoPlayer1" src={files[0].preview} type="video/mp4" /> */}
                  </video>
                </Grid>
              )}
              {projects.project?.service_type !== undefined && projects.project?.service_type.indexOf("_training") == -1 && secSetupVidCreationTime()}
            </Grid>
            {projects.project?.service_type !== undefined && secSetupVidStartPoint()}
          </div>
        );
    ...
    if (chosenItem === "apiVideo")
      return (
        ...
        >
          ...
          <br /> {t("You’ll be notified in the notification window when it is completed")}
        </div>
      );
        ...
        <>
          ...
          <div style={{ overflow: "hidden" }}>
            <div style={{ alignSelf: "center", width: "100%" }}>
              ...
              >
                <JSONPretty id="json-pretty" data={objectJson} className={classes.predictResultJson} mainStyle="color:#ffffff" keyStyle="color:#1BC6B4" valueStyle="color:#0A84FF"></JSONPretty>
              </div>
            </div>
          </div>
        </>
        ...
        return (
          <div style={{ alignSelf: "center", width: "100%" }} id="resultDiv">
            {selectedObjectTab === "image" ? (
              <>
                <Button id="resultDownload" className={classes.defaultHighlightButton} onClick={downloadImage}>
                  DOWNLOAD
                </Button>
              ...
              >
                <JSONPretty id="json-pretty" data={objectJson} mainStyle="color:#ffffff" keyStyle="color:#1BC6B4" valueStyle="color:#0A84FF"></JSONPretty>
              </div>
            )}
          </div>
        );
                ...
                >
                  <ResponsiveContainer width="100%" height={250}>
                    ...
                    >
                      ...
                      <Tooltip content={<CustomTooltip />} cursor={{ fill: currentThemeColor.background2 }} />
                      <CartesianGrid stroke={"#999999"} strokeDasharray="3 3" />
                      <YAxis dataKey="name" width={50} height={10} type="category" style={{ fontSize: "0.75rem" }} />
                      <XAxis type="number" domain={[0, 100]} style={{ fontSize: "0.75rem" }} />
                      <ReferenceLine x={0} stroke="rgb(24, 160, 251)" strokeWidth={3} alwaysShow={true} />
                      <Bar dataKey="value" type="monotone" barSize={30} fill={currentThemeColor.primary2} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </>
  ...
  const handleOutputResultText = (preVal) => {
    ...
    if (projects?.project?.valueForNorm && multiplyAverage && typeof tempResultValue === "number") tempResultValue = tempResultValue * projects?.project?.valueForNorm;
    return tempDiscriminant ? +(Math.round(+tempResultValue * 1000) / 1000) : tempResultValue;
  };

  const secAPIActionBtns = () => {
    let serviceType = projects.project?.service_type;
    let isNewUpload = serviceType && (serviceType.indexOf("_training") != -1 || serviceType.indexOf("offline_") != -1) && chosenItem == "apiVideo";
    ...
    return (
      ...
      >
        <GridItem xs={4}>
          <Button id="cancelBtn" className={classes.defaultF0F0OutlineButton} style={{ width: "100%", height: "35px", marginRight: "20px" }} onClick={closeModal}>
            {t("Cancel")}
          </Button>
        </GridItem>
        <GridItem xs={4}>
          ...
          {chosenItem === "apiVideo" ? (
            <GridContainer>
              <GridItem xs={12} style={{ fontSize: "25px" }}>
                {models.model && models.model.name && models.model.name.toUpperCase()}
              </GridItem>
              <GridItem
                xs={12}
                style={{
              ...
              <GridItem xs={12}>{apiLoading === "done" ? renderApiResult() : renderContents()}</GridItem>
            </GridContainer>
          ) : (
            <GridContainer style={{ minHeight: "488px" }}>
              ...
              <GridItem xs={apiLoading === "done" ? 5 : 8}>
                <div className={classes.flexContent}>
                  ...
                  {apiLoading !== "done" && ((chosenItem === "api" && hasRecordsData) || randomFiles.length > 0) && (
                    <GridItem xs={6}>
                      <Button
              ...
              ></GridItem>
              <GridItem xs={apiLoading === "done" ? 6 : 3} style={{ paddingLeft: "0" }}>
                <div className={classes.content}>
                  <div style={{ display: "flex" }}>
                    ...
                    {chosenItem === "apiImage" && (
                      <div
                        ...
                        style={{
                          marginLeft: "16px",
                          fontWeight: "700",
                          color: selectedObjectTab === "image" ? "var(--secondary1)" : "var(--textWhite87)",
                        }}
                        // className={
                        //   selectedObjectTab === "image"
                        //     ? classes.selectedTab
                    ...
                    {objectJson && (
                      <div onClick={() => handleChangeTab("json")} className={selectedObjectTab === "json" ? classes.selectedTab : classes.notSelectedTab}>
                        {t("Coordinate value")}
                      </div>
                    )}
                    {xaiImage && (
                      <div onClick={() => handleChangeTab("xai")} className={selectedObjectTab === "xai" ? classes.selectedTab : classes.notSelectedTab}>
                        XAI
                      </div>
                    )}
                  </div>
              ...
              <GridItem xs={apiLoading === "done" ? 5 : 8} style={{ paddingRight: "0" }}>
                {renderContents()}
                <p>{Object.values(outliarInfo).filter((infoValue) => infoValue === true).length > 0 ? `* ${t("The red letters are outliar values")}` : ""}</p>
              </GridItem>
              <GridItem
                xs={1}
                style={{
              ...
              <GridItem xs={apiLoading === "done" ? 6 : 3} style={{ paddingLeft: "0" }}>
                <div className={classes.content}>
                  <div style={{ overflow: "hidden" }}>{apiLoading === "done" && renderApiResult()}</div>
                </div>
              </GridItem>
            </GridContainer>
          )}
