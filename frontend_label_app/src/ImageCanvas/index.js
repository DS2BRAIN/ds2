...
type Props = {
  ...
  onDeletePolygonPoint: (Polygon, index: number) => any,
  onAddPolygonPoint: (Polygon, point: [number, number], index: number) => any,
  onBeginMovePolylinePoint: (Polyline, index: number) => any,
  onAddPolylinePoint: (Polyline, point: [number, number], index: number) => any,
...
export default ({
  ...
  const selectedRegion = regions.filter(
    (v) => v.hilighted && v.type === "polygon"
  )
  // useEffect(() => {
  //   if (selectedRegion.isMagicUpdateDone) {
  ...
  //     context.strokeStyle = selectedRegion.color
  //     context.beginPath()
  //     if (selectedRegion.points && selectedRegion.points[0]) {
  //       context.moveTo(selectedRegion.points[0][0] * iw, selectedRegion.points[0][1] * ih)
  //       for (const point of selectedRegion.points) {
  ...
  //     context.restore()
  //   }
  // }, [selectedRegion.isMagicUpdateDone])
  useLayoutEffect(() => {
    ...
    for (const region of regions.filter(
      ...
      switch (region.type) {
        case "point": {
          ...
          context.arc(region.x * iw, region.y * ih, 4, 0, 2 * Math.PI)
          context.stroke()

          context.arc(region.x * iw, region.y * ih, 3, 0, 2 * Math.PI)
          //'invisible' 일 때 -> yellow로 채우기
        ...
        case "polygon": {
          ...
          context.strokeStyle = region.color
          context.beginPath()
          if (region.points && region.points[0]) {
            context.moveTo(region.points[0][0] * iw, region.points[0][1] * ih)
            for (const point of region.points) {
              context.lineTo(point[0] * iw, point[1] * ih)
            }
    ...
    >
      ...
      {imageLoaded && (
        <RegionSelectAndTransformBoxes
          ...
          onDeletePolygonPoint={onDeletePolygonPoint}
          onAddPolygonPoint={onAddPolygonPoint}
          onBeginMovePolylinePoint={onBeginMovePolylinePoint}
      ...
      >
        <>
          <canvas className={classes.canvas} ref={canvasEl} id="mycanvas" />
