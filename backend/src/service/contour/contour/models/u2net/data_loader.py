...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        image = io.imread(self.img_path_list[idx])
        label = io.imread(self.label_path_list[idx]) if self.label_path_list else np.zeros(image.shape)

        image_dim = len(image.shape)
        label_dim = len(label.shape)
