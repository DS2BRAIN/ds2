...
async def getAsyncTask(request: Request, token: str, provider: str = 'DS2.ai'):
    return EventSourceResponse(manageTaskClass.get_async_tasks(token, provider, request))
...
def updateAsyncTask(asnyctaskId: int, token: str, response: Response):
    response.status_code, result = manageTaskClass.putAsyncTasks(token, asnyctaskId)
    return result
