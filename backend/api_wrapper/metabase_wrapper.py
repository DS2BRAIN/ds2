...
class MetabaseAPI:
    ...
    def authenticate(self):
        ...
        res = requests.post(self.domain + '/api/session', json=conn_header, auth=self.auth, timeout=5)
        if not res.ok:
            raise Exception(res)
    ...
    def add_database(self, conn_name=None, conn_uri=None, verbose=False):

        params = {
            "engine": "mongo",
            "name": conn_name,
            "details": {
                "use-connection-uri": True,
                "conn-uri": conn_uri,
        ...
        res = self.post(f'/api/database', json=params)
        if res:
            self.verbose_print(verbose, 'The database was connected successfully.')
        return res

    def add_group(self, name=None, verbose=False):

        params = {
            "name": str(name)
        }
        res = self.post(f'/api/permissions/group', json=params)
        if res:
            self.verbose_print(verbose, 'The group was created successfully.')
            return res
        else:
            raise ValueError('add group {} failed'.format(name))

    def add_user(self, first_name=None, last_name=None, email=None, password=None, verbose=False):
        ...
        res = self.post(f'/api/user', json=params)
        if res:
            self.verbose_print(verbose, 'The user was created successfully.')
            return res
        else:
            raise ValueError('add user {} failed'.format(email))

    def put_user(self, user_id, old_password, new_password):

        params = {
            'old_password': old_password,
            'password': new_password
        }
        res = self.put(f'/api/user/{user_id}/password', json=params)
        if res == 200:
            return res
        elif res == 409:
            raise ValueError('The put user is not valid')
        else:
            raise Exception('The put user failed')

    def add_user_to_group(self, user_id, group_id, verbose=False):
        params = {
            "user_id": user_id,
            "group_id": group_id
        }
        res = self.post(f'/api/permissions/membership', json=params)
        if res:
            self.verbose_print(verbose, 'The user was added to group successfully.')
            return res
        else:
            raise ValueError('add user {} to group {} failed'.format(user_id, group_id))

    def sync_database(self, database_id, verbose=False):

        res = self.post(f'/api/database/{database_id}/sync_schema')
        if res.get('status') == 'ok':
            self.verbose_print(verbose, 'The database sync successfully.')
            return True
        else:
            self.verbose_print(verbose, 'The database sync failed.')
            return False

    def get_permission(self, verbose=False):

        res = self.get("/api/permissions/graph")
        if type(res) == dict:
            self.verbose_print(verbose, 'The permissions retrieve successfully.')
            return res
        else:
            raise ValueError('The permission retrieve failed')

    def put_permission(self, permission_body, verbose=False):

        res = self.put('/api/permissions/graph', json=permission_body)

        if res == 200:
            self.verbose_print(verbose, 'The permissions update successfully.')
            return res
        elif res == 409:
            raise ValueError('The permission revision is not valid')
        else:
            raise Exception('The permission update failed')

    def get_group_id(self, user_id, verbose=False):

        res = self.get("/api/permissions/group")
        if type(res) == list:
            group_id = None
            for item in res:
                if user_id == item.get('name'):
                    group_id = item.get('id')
            if group_id is None:
                raise ValueError('The group name is not exists.')
            self.verbose_print(verbose, 'The group id retrieve successfully.')
            return group_id
        else:
            raise ValueError('The group id retrieve failed')
...
if __name__ == "__main__":
    ...
    try:
        view_name = f"Test2"
        db_class.delete_collection_by_name(view_name)

        util_class.metabase_admin.get_group_id(15)
        # 사용자 추가
        userInfo = {
            'email': 'test1632@dslab.global',
            'id': 18
        }
        raw_password = 'test12345'
        meta_email = userInfo.get('email')
        meta_names = meta_email.split('@')
        meta_first_name = meta_names[0]
        meta_last_name = meta_names[1]

        # 그룹 추가
        new_group = util_class.metabase_admin.add_group(userInfo.get('id'))
        group_id = new_group.get('id')
        new_meta_user = util_class.metabase_admin.add_user(meta_first_name, meta_last_name, meta_email,
                                                           raw_password)
        meta_user_id = new_meta_user.get('id')
        util_class.metabase_admin.add_user_to_group(meta_user_id, group_id)

        # 뷰 추가
        dataconnector_id = 13749
        dataconnector_name = 'test2'

        database_id = util_class.metabase_admin.get_item_id('database', util_class.metabase_database_name)
        table_name = f'{dataconnector_name}'
        try:
            table_id = util_class.metabase_admin.get_item_id('table', table_name)
        except ValueError:
            db_class.create_ds2data_view(view_name=table_name, dataconnector_list=[dataconnector_id])
            util_class.metabase_admin.sync_database(database_id)
            table_id = util_class.metabase_admin.get_item_id('table', table_name)

        # 권한 추가
        while 1:
            permission_body = util_class.metabase_admin.get_permission()
            permission_body['groups'][group_id] = {
                database_id: {
                    'data': {
                        'schemas': {
                            '': {
            ...
            try:
                permission = util_class.metabase_admin.put_permission(permission_body)
                break
            # revision 값이 달라서 실패하는 경우
            except ValueError:
                continue
            except Exception:
                raise Exception
        ...
        # 대시보드 링크 반환
        # mb = MetabaseAPI(util_class.metabase_url, meta_email, raw_password)

        # metabase_url = 'http://localhost:3000'
        # user_name = 'metabase@meta.base'
        # password = 'metabase1'

        # mb = MetabaseAPI(metabase_url, user_name, password)
        ...
        try:
            ...
            db_class.create_ds2data_view(view_name=table_name, dataconnector_list=[dataconnector_id])
            # 스키마 동기화
            database_id = mb.get_item_id('database', 'mongo')
            mb.sync_database(database_id)
            # 뷰 이름 -> 뷰 id
            table_id = mb.get_item_id('table', table_name)
        # 동기화 상태 확인
        table = mb.get_item_info('table', table_id)
        if table.get('initial_sync_status') == 'complete':
            # metabase redirection
            dashboard_url = f"{metabase_url}/auto/dashboard/table/{table_id}"
            print(dashboard_url)
        else:
            print("분석 중 입니다.")
            # incomplete 인 경우 다시 요청

        # 엔드포인트 생성
        # 프론트에 연동(버튼 추가)

        # 데이터커넥터 -> 분석 클릭 -> 메타베이스 탭으로 이동
        # 프로젝트 -> 모델 상세보기 -> 학습데이터로 분석/예측데이터로 분석 클릭 -> 메타베이스 탭으로 이동
    except ValueError as e:
        # 조회 실패 에러
        print("error")