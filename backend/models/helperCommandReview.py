...
class HelperCommandReview():
    ...
    @wrapper
    def getOneCommandReviewById(self, rowId, raw=False):
        collection = commandReviewTable.get_or_none(commandReviewTable.id == rowId)
        if collection:
            return collection.__dict__['__data__'] if not raw else collection
        return collection

    @wrapper
    def getCommandReviewsByUserId(self, tableInstance, user_id, sorting = 'created_at', tab = 'all', desc = False, searching = '', start = 0, count = 10):
    ...
    @wrapper
    def updateCommandReview(self, rowId, data):
        data["updated_at"] = datetime.datetime.utcnow()
        # if data.get("status") != None:
        #     collectionData = self.getOneCommandReviewById(rowId)
        #     collectionData.update(data)
        #     collectionhistoriesTable.create(**(collectionData))
        return commandReviewTable.update(**data).where(commandReviewTable.id == rowId).execute()